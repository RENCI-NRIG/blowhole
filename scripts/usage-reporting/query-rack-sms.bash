...
# insert in DB
rrdtool update rrd/$SM-sm.rrd -t local_vlan:transit_vlan:global_vlan:mp_vlan:vm:bm:lun N:$LOCAL_VLAN:$TRANSIT_VLAN:$GLOBAL_VLAN:$MP_VLAN:$VM:$BM:$LUN
DATE=`date`
echo $DATE > last_result_sms
echo $SM $LOCAL_VLAN:$TRANSIT_VLAN:$GLOBAL_VLAN:$MP_VLAN:$VM:$BM:$LUN >> last_result_sms
...
show reservations for all actor $SM-sm state active filter "lun"
  ...
  rrdtool update rrd/$SM-sm.rrd -t local_vlan:transit_vlan:vm:bm:lun N:$LOCAL_VLAN:$TRANSIT_VLAN:$VM:$BM:$LUN
  echo $SM $LOCAL_VLAN:$TRANSIT_VLAN:$VM:$BM:$LUN >> last_result_sms
done
